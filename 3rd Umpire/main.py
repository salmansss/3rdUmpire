...
stream = cv2.VideoCapture("stumping.mp4")
flag = True
def play(speed):
    global flag
...
def pending(decision):
    ...
    frame = cv2.cvtColor(cv2.imread("sponsor.jpg"), cv2.COLOR_BGR2RGBA)
    frame = imutils.resize(frame, width=SET_WIDTH, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0,0, image=frame, anchor=tkinter.NW)
...
cv_img = cv2.cvtColor(cv2.imread("welcome1.jpg"), cv2.COLOR_BGR2RGBA)
